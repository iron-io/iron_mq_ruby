  ...
  module Swagger
    class Request
      ...
      def initialize(http_method, path, attributes={})
        ...
        # Because we use "oauth" we have to add the OAuth key in front of it
        attributes[:headers].merge!({:Authorization => "OAuth " + Swagger.configuration.auth_token})
      ...
      def update_params_for_auth!
        (@auth_names || []).each do |auth_name|
          ...
          when 'oauth_token'
            @headers ||= {}
            @headers['Authorization'] = get_api_key_with_prefix('Authorization')
